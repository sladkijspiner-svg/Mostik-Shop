...
app.listen(PORT, () => {
  ...
  // ВАЖНО: автозапуск при старте сервера был отключён ещё раз — сборка
  // мировой статистики упирается в лимит памяти этого тарифа Railway и
  // регулярно падает по Out Of Memory. Прогресс сохраняется на диск (см.
