...
// Считается один раз в фоне (см. server/analytics.js) и кэшируется — сам
// запрос просто отдаёт то, что уже посчитано (плюс флаг "идёт пересчёт",
// если фонового прохода по всей базе ещё не было или он ещё не закончился).
...
app.listen(PORT, () => {
  ...
  // ВАЖНО: автозапуск полного обхода всей базы (~6500+ фигурок) при каждом
  // старте сервера роняет процесс по памяти на маленьком тарифе Railway —
  // сервер падал по Out Of Memory прямо посреди сканирования, а после
  // перезапуска падал снова и снова (бесконечный краш-луп), из-за чего
  // переставало работать вообще всё, не только аналитика. Поэтому
  // автозапуск отключён — статистику теперь считаем только вручную,
  // командой /rebuildstats, когда это удобно и можно последить за
  // потреблением памяти.
  // analytics.maybeAutoBuild();
});
