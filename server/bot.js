...
bot.onText(/^\/rebuildstats/, msg => {
  ...
  // Ручной запуск сбрасывает счётчик автопопыток — после того как админ сам
  // вмешался, у автопродолжения (см. analytics.js/maybeAutoResumeBuild)
  // снова должен быть полный запас попыток на случай новых падений.
  analytics.resetAutoResume();
  bot.sendMessage(chatId, `Начинаю (или продолжаю) сбор мировой статистики за ${key} — это может занять время, напишу, когда закончу. Если сервер вдруг перезапустится посреди процесса — можно ничего не делать, сервер попробует продолжить сам (до нескольких раз), а если не получится — напишу и попрошу отправить команду ещё раз.`);
